...
test 0 -ne $(find . -maxdepth 1 -name ".library" | wc -l) || $(echo "Error: Marker not found" >&2; exit 1)
...
while [ -e "$LIBRARY_DIR/books/${ADD_BOOK_NAME}${ADD_BOOK_SUFFIX}" ]; do
...
cp "$ADD_BOOK_SOURCE" "$LIBRARY_DIR/.tmp/$(basename "$ADD_BOOK_FILENAME")"

# move book from tmp to a books directory
mv "$LIBRARY_DIR/.tmp/$(basename "$ADD_BOOK_FILENAME")" "$LIBRARY_DIR/books/$ADD_BOOK_FILENAME"
chmod 664 "$LIBRARY_DIR/books/$ADD_BOOK_FILENAME"

# creates price file
if [ -n "$ADD_BOOK_PRICE" ] && [ "$ADD_BOOK_PRICE" != "-" ]; then
    echo "$ADD_BOOK_PRICE" > "$LIBRARY_DIR/prices/${ADD_BOOK_FILENAME}.price"
    chmod 644 "$LIBRARY_DIR/prices/${ADD_BOOK_FILENAME}.price"
fi

# author workflow
for AUTHOR in $ADD_BOOK_AUTHORS; do
    # author directory creation
    AUTHOR_DIR="$LIBRARY_DIR/authors/$AUTHOR"
    ...
    # hard link creation
    ln "$LIBRARY_DIR/books/$ADD_BOOK_FILENAME" "$AUTHOR_DIR/$ADD_BOOK_FILENAME"
done